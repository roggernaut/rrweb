...
- BREAKING: `@rrweb/rrweb-plugin-privacy-detectors`'s API and the
  ...
  honored) -- see the guide's migration section for the full list.
- Add an opt-in, versioned `privacyPolicy` with `strict`/`balanced`/`legacy`
  ...
  options remain the `legacy` default.
