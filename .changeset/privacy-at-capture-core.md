...
- BREAKING: `@rrweb/rrweb-plugin-privacy-detectors` and the
  ...
  enforced (e.g. protected inputs, mask/unmask ties, foreign unmask tokens
  never honored), and a few APIs change shape (`needMaskingText`,
  `CompiledPrivacyPolicy`) -- see the guide's migration section for the full
  list.
- Add an opt-in, versioned `privacyPolicy` with `strict`, `balanced`, and
  `legacy` presets, consistently protecting text, form values, and sensitive
  attributes across full snapshots and incremental mutations; existing rrweb
  masking options remain the backwards-compatible `legacy` default.
- Recognize the vendor-neutral `data-privacy="exclude|mask|allow"` HTML
  binding and common cross-vendor masking class names directly in markup
  under `balanced`/`strict`; selector-based policy `rules` work under every
  preset, including `legacy`. CSS is never masked, on any preset.
