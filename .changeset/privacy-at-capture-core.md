...
primitives; CSS is never masked; canvas masking forces the FPS capture path;
selector and config errors fail closed. BREAKING (@rrweb/types):
`ImageBitmapDataURLWorkerParams` is a union; privacy rule `style`,
`classification`, heuristic detectors, and the `'custom'` preset are removed
from the policy schema, and the `@rrweb/rrweb-plugin-privacy-detectors`
package is removed along with the `RecordPlugin.applyPrivacyPolicy` hook it
was the only consumer of. Both return in a follow-up change that reintroduces
detectors as an explicitly experimental, opt-in layer.
...
- The preset's masked attributes (`title`, `placeholder`, `aria-label`) now
  ...
  three behave identically on both the snapshot and the mutation path.
  `strict`'s media-source dropping is not escapable this way.
...
- BREAKING: masking a form value now also suppresses the `selected` flag on
  ...
  `balanced`/`strict` the chosen option was recorded verbatim, contradicting
  the documented form-value guarantee. Both decisions now go through one
  exported predicate, `shouldMaskInput`.
