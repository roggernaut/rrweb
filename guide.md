...
| Hotjar                   |                                                                                                                      | `[data-hj-suppress]`, `.data-hj-suppress`                                     | "How to Suppress Text, Images, Videos and User Input" (both spellings documented; text is masked in place there but images/videos are placeholdered too, and our mask verb leaves image sources readable, so block)                                                                            |
| Microsoft Clarity        | `[data-clarity-mask]`                                                                                                |                                                                               | "Clarity masking"                                                                                                                                                                                                                                                                              |
| Smartlook                | `[data-sl="mask"]`, `[data-recording-sensitive]` (legacy)                                                            | `[data-sl="exclude"]`, `[data-recording-disable]` (legacy)                    | Smartlook web Privacy API; the legacy attributes survive only in archived docs (`smartlook.github.io` via web.archive.org), where `data-recording-sensitive` masked text and also ignored input values/events -- so it is in the ignore list too                                               |
...
| Heap                     |                                                                                                                      | `[data-heap-redact-text]`, `[data-heap-redact-attributes]`                    | Heap "What privacy settings does Session replay inherit": both redact attributes redact the entire element in their replay, so block. `[heap-ignore]` suppresses autocapture events only and is absent from the replay-inheritance list, so it is in the ignore list. Attribute forms only     |
| Mouseflow                | `[data-mf-replace-inner]` (inner text placeholdered, structure kept)                                                 | `.mf-excluded`, `.mf-masked`, `[data-mf-replace]`                             | Mouseflow "Excluding, masking and replacing content via code": `mf-masked` is "not recorded at all" and `data-mf-replace` swaps the subtree for its placeholder value, so both block                                                                                                           |
...
| Userback                 |                                                                                                                      | `.userback-block`                                                             | Userback "Session replay": `userback-block` ignores the element and its children entirely; `.userback-ignore` keeps the element rendered and ignores its user input, so it is in the ignore list                                                                                               |
...
| Glassbox                 | `.cls_mask` (input values)                                                                                           |                                                                               | Observed-only: no public or archived docs; read from the shipped `detector-dom.min.js` SDK, where `cls_mask` is the one hard-coded owner-facing token and everything else is console configuration                                                                                             |
| SessionStack             | `.sessionstack-sensitive`                                                                                            |                                                                               | SessionStack "Sensitive data" (archived copy; the live page blocks non-browser clients): text masked client-side by replacing each alphanumeric character with X                                                                                                                               |
| Session Rewind           |                                                                                                                      | `[data-sr-redact]`                                                            | Session Rewind "Privacy settings" (Notion docs): the documented verb is "exclude"/"redact" for arbitrary elements with the replay rendering unspecified, so the stricter verb                                                                                                                  |
...
constant `ignore_class`; no attribute form ships), Highlight's
`.highlight-ignore` (their `ignoreClass`), Heap's `[heap-ignore]`
(autocapture-event suppression only; absent from their replay-inheritance
list), and Userback's `.userback-ignore` (element rendered, its user input
ignored). Each was verified, from the vendor's source or official docs, to
suppress events without hiding content; honoring one can only reduce what
is recorded, and the annotated element's content records normally, exactly
as it does under that vendor. Smartlook's legacy
`[data-recording-sensitive]` is the one dual-slot token: it masked text
and ignored input events there, so it is in both the mask and ignore
lists.
...
`.lo-not-sensitive`, `[data-dtrum-allow]`, `[data-qm-allow]`), Smartlook's
legacy `[data-recording-ignore]`, whose value-dependent semantics
(`"mask"` or `"events"`) survive only in archived docs, and
...
- `ImageBitmapDataURLWorkerParams` (`@rrweb/types`) is now a union; privacy
  rule `style`/`classification`, custom detectors, and the `'custom'` preset
  are removed from the policy schema.
- Heuristic detection now masks whole values (no character-range masking) and
  scans page text only; input values are never scanned by detectors, on any
  path -- see [Heuristic PII detectors](#heuristic-pii-detectors).
